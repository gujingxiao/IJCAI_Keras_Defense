...
def main():
    ...
    #    count += 1
    #
    #for layer in base_model.layers[0:32]:
    #    print(layer.name, layer.output_shape)
    #    layer.trainable = False

    if use_MoE == True:
        x = DenseMoE(512, n_experts=2, expert_activation='relu', gating_activation='softmax')(base_model.output)
        x = Dropout(0.2)(x)
        predict = Dense(number_of_classes, activation='softmax')(x)
    else:
        output = Dropout(0.2)(base_model.output)
        predict = Dense(number_of_classes, activation='softmax')(output)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--finetune_dir", default="./models/ijcai_densenet121")
    ...
    # 使用哪个网络结构
    argparser.add_argument("--model_type", default="densenet121")
